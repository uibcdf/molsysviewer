...
def run_benchmarks(iterations: int = 50, verbose: bool = True) -> str:
    ...
    for key, stats in load_results.items():
        lines.append(
            f"| `{key}` | {stats['min']:.2f} | {stats['max']:.2f} | {stats['mean']:.2f} | {stats['std']:.2f} |"
        )
    lines.append("")
    ...
    for name, stats in serial_results.items():
        lines.append(
            f"| `{name}` | {stats['min']:.4f} | {stats['max']:.4f} | {stats['mean']:.4f} | {stats['std']:.4f} |"
        )
    lines.append("")
    ...
    lines.append(
        f"| `Baseline (None)` | {baseline['min']:.2f} | {baseline['max']:.2f} | {baseline['mean']:.2f} | {baseline['std']:.2f} | — | — |"
    )
