...
function eachVolumeLoci(loci, volume, props, apply) {
    ...
    if (volume_1.Volume.isLoci(loci)) {
        ...
        if (int_1.Interval.is(loci.instances)) {
            const start = int_1.Interval.start(loci.instances) * cellCount;
            const end = int_1.Interval.end(loci.instances) * cellCount;
            ...
            for (let i = 0, il = loci.instances.length; i < il; ++i) {
                ...
                if (apply(int_1.Interval.ofBounds(offset, offset + cellCount)))
                    changed = true;
            }
        }
    }
        ...
        if (props === null || props === void 0 ? void 0 : props.isoValue) {
            ...
            for (let i = 0, il = data.length; i < il; ++i) {
                if ((0, common_1.equalEps)(v, data[i], eps)) {
                    int_1.OrderedSet.forEach(loci.instances, j => {
                        const offset = j * cellCount;
                        if (apply(int_1.Interval.ofSingleton(offset + i)))
                            changed = true;
                    });
                }
            }
        ...
        for (const { indices, instances } of loci.elements) {
            if (int_1.Interval.is(indices)) {
                int_1.OrderedSet.forEach(instances, j => {
                    const offset = j * cellCount;
                    if (apply(int_1.Interval.offset(indices, offset)))
                        changed = true;
                });
            }
            else {
                int_1.OrderedSet.forEach(indices, v => {
                    int_1.OrderedSet.forEach(instances, j => {
                        const offset = j * cellCount;
                        if (apply(int_1.Interval.ofSingleton(offset + v)))
                            changed = true;
                    });
                });
            }
        ...
        if (props === null || props === void 0 ? void 0 : props.segments) {
            for (const { segments, instances } of loci.elements) {
                if (int_1.OrderedSet.areIntersecting(segments, props.segments)) {
                    int_1.OrderedSet.forEach(instances, j => {
                        const offset = j * cellCount;
                        if (apply(int_1.Interval.ofBounds(offset, offset + cellCount)))
                            changed = true;
                    ...
                    for (let i = 0, il = d.length; i < il; ++i) {
                        if (s.includes(d[i])) {
                            for (let j = 0, _j = int_1.OrderedSet.size(instances); j < _j; j++) {
                                const offset = j * cellCount;
                                if (apply(int_1.Interval.ofSingleton(i + offset)))
                                    changed = true;
                            }
                        }
                    }
