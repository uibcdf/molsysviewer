...
(function (Plane3D) {
    ...
    function distanceToSphere3D(plane, sphere) {
        return distanceToPoint(plane, sphere.center) - sphere.radius;
    }
    Plane3D.distanceToSphere3D = distanceToSphere3D;
    ...
    function intersectRay3D(out, plane, ray) {
        const denominator = vec3_1.Vec3.dot(plane.normal, ray.direction);
        if (denominator === 0)
            return false; // parallel
        const t = -(vec3_1.Vec3.dot(plane.normal, ray.origin) + plane.constant) / denominator;
        if (t < 0)
            return false; // behind the ray
        vec3_1.Vec3.scaleAndAdd(out, ray.origin, ray.direction, t);
        return true;
    }
    Plane3D.intersectRay3D = intersectRay3D;
