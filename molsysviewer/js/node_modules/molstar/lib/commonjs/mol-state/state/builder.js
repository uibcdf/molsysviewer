...
(function (StateBuilder) {
    ...
    function buildTree(state, options) {
        if (!state.state || state.state.tree === state.editInfo.sourceTree) {
            const ret = state.tree.asImmutable();
            if (options === null || options === void 0 ? void 0 : options.useHashVersion) {
                immutable_1.StateTree.setParamHashVersion(ret, getAffectedRefs(state));
            }
            return ret;
        }
    ...
    class Root {
        ...
        getTree(options) { return buildTree(this.state, options); }
        commit(options) {
            if (!this.state.state)
    ...
    class To {
        ...
        update(paramsOrTransformer, provider) {
            ...
            if (provider) {
                ...
                params = (0, produce_1.produce)(old.params, provider);
            }
            else {
                params = typeof paramsOrTransformer === 'function'
                    ? (0, produce_1.produce)(this.state.tree.transforms.get(this.ref).params, paramsOrTransformer)
                    : paramsOrTransformer;
            }
