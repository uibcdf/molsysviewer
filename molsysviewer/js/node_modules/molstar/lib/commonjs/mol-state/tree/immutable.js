...
const transient_1 = require("./transient.js");
var StateTree;
(function (StateTree) {
    class Impl {
    ...
    function setParamHashVersion(tree, refs) {
        for (const ref of refs) {
            ...
            if (transform) {
                transform_1.StateTransform.setParamsHashVersion(transform);
            }
        }
    }
    StateTree.setParamHashVersion = setParamHashVersion;
    /** Re-use parameters of transforms with the same ref, transformer, and version */
    function reuseTransformParams(destination, source) {
        const srcMap = new Map(source.transforms.map(t => [t.ref, t]));
        for (const dest of destination.transforms) {
            const src = srcMap.get(dest.ref);
            if (!src)
                continue;
            if (dest.transformer !== src.transformer || dest.version !== src.version)
                continue;
            dest.params = src.params;
        }
    }
    StateTree.reuseTransformParams = reuseTransformParams;
