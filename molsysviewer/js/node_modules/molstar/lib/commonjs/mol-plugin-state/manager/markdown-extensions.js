...
exports.MarkdownExtensionManager = exports.BuiltInMarkdownExtension = void 0;
exports.defaultParseMarkdownCommandArgs = defaultParseMarkdownCommandArgs;
...
const state_snapshots_1 = require("../animation/built-in/state-snapshots.js");
exports.BuiltInMarkdownExtension = [
    {
        name: 'center-camera',
    ...
    },
    {
        ...
        execute: ({ event, args, manager }) => {
            ...
            if (!reprs.length)
                return;
            const spheres = reprs.map(c => (0, focus_object_1.getCellBoundingSphere)(manager.plugin, c.transform.ref)).filter(s => !!s);
            if (!spheres.length)
                return;
    ...
    },
    {
        ...
        execute: ({ event, args, manager }) => {
            ...
            const structures = manager.plugin.state.data.selectQ(q => q.rootsOfType(objects_1.PluginStateObject.Molecule.Structure));
            if (event === 'mouse-enter') {
                ...
                for (const structure of structures) {
                    if (!((_a = structure.obj) === null || _a === void 0 ? void 0 : _a.data))
                        continue;
                    const selection = query(new structure_1.QueryContext(structure.obj.data));
            ...
            if (event === 'click') {
                ...
                const decorated = structures.map(s => mol_state_1.StateSelection.getDecorated(manager.plugin.state.data, s.transform.ref));
                const spheres = decorated.map(s => {
                    var _a;
                    if (!((_a = s.obj) === null || _a === void 0 ? void 0 : _a.data))
                        return undefined;
                    const selection = query(new structure_1.QueryContext(s.obj.data));
                    if (structure_1.StructureSelection.isEmpty(selection))
                        return;
                    const loci = structure_1.StructureSelection.toLociWithSourceUnits(selection);
                    return structure_1.StructureElement.Loci.getBoundary(loci).sphere;
                }).filter(s => !!s);
                if (spheres.length) {
    ...
    },
    {
        ...
        execute: ({ event, args, manager }) => {
            if (event !== 'click' || !('play-transition' in args))
                return;
            manager.plugin.managers.animation.play(state_snapshots_1.AnimateStateSnapshotTransition, {});
        }
    },
    {
        name: 'play-snapshots',
...
class MarkdownExtensionManager {
    ...
    constructor(plugin) {
        this.plugin = plugin;
        this.state = {
            audioPlayer: new rxjs_1.BehaviorSubject(null),
        };
        this.extension = [];
        this.refResolvers = {
        ...
        for (const command of exports.BuiltInMarkdownExtension) {
            this.registerExtension(command);
        }
    }
}
exports.MarkdownExtensionManager = MarkdownExtensionManager;
...
function findRepresentations(plugin, cells) {
    if (!cells.length)
        return [];
    return plugin.state.data.selectQ(q => q.byValue(...cells).subtree().filter(c => objects_1.PluginStateObject.isRepresentation3D(c.obj)));
}
function defaultParseMarkdownCommandArgs(input) {
