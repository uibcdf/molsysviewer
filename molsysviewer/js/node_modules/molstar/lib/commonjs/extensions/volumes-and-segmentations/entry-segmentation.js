...
const misc_1 = require("../../mol-plugin-state/transforms/misc.js");
...
class VolsegLatticeSegmentationData {
    ...
    makeLoci(segments) {
        ...
        if (!wholeLoci || !volume_1.Volume.Segment.isLoci(wholeLoci))
            return undefined;
        const elements = [{
                segments: int_1.SortedArray.ofUnsortedArray(segments),
                instances: interval_1.Interval.ofLength(wholeLoci.volume.instances.length)
            }];
        return { loci: volume_1.Volume.Segment.Loci(wholeLoci.volume, elements), repr: repr };
    }
