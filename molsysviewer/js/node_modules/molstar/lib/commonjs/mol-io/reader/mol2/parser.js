...
async function handleAtoms(state) {
    ...
    const status_bitsTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_atoms * 2);
    ...
    const ret = {
        ...
        status_bits: maxColumnCount > 9 ? (0, token_1.TokenColumnProvider)(status_bitsTokens)(db_1.Column.Schema.str) : undefStr,
    };
    return ret;
}
async function handleBonds(state) {
    ...
    const status_bitsTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_bonds * 2);
    const undefStr = db_1.Column.Undefined(molecule.num_bonds, db_1.Column.Schema.str);
    tokenizer.position = initialTokenizerPosition;
    tokenizer.lineNumber = initialTokenizerLineNumber;
    let maxColumnCount = 0;
    const tokenIndexToColumn = [
    ...
    const ret = {
        ...
        status_bits: maxColumnCount > 4 ? (0, token_1.TokenColumnProvider)(status_bitsTokens)(db_1.Column.Schema.str) : undefStr,
    };
    return ret;
}
async function handleSubstructures(state) {
    const { tokenizer, molecule } = state;
    if (molecule.num_subst === 0)
        return;
    ...
    const initialTokenizerLineNumber = tokenizer.lineNumber;
    // columns
    const subst_idTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_subst * 2);
    const subst_nameTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_subst * 2);
    const root_atomTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_subst * 2);
    const subst_typeTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_subst * 2);
    const dict_typeTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_subst * 2);
    const chainTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_subst * 2);
    const sub_typeTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_subst * 2);
    const inter_bondsTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_subst * 2);
    const status_bitsTokens = tokenizer_1.TokenBuilder.create(tokenizer.data, molecule.num_subst * 2);
    const undefInt = db_1.Column.Undefined(molecule.num_subst, db_1.Column.Schema.int);
    const undefStr = db_1.Column.Undefined(molecule.num_subst, db_1.Column.Schema.str);
    tokenizer.position = initialTokenizerPosition;
    tokenizer.lineNumber = initialTokenizerLineNumber;
    let maxColumnCount = 0;
    const tokenIndexToColumn = [
    ...
    const ret = {
        ...
        subst_id: (0, token_1.TokenColumnProvider)(subst_idTokens)(db_1.Column.Schema.int),
        subst_name: (0, token_1.TokenColumnProvider)(subst_nameTokens)(db_1.Column.Schema.str),
        root_atom: (0, token_1.TokenColumnProvider)(root_atomTokens)(db_1.Column.Schema.int),
        subst_type: maxColumnCount > 3 ? (0, token_1.TokenColumnProvider)(subst_typeTokens)(db_1.Column.Schema.str) : undefStr,
        dict_type: maxColumnCount > 4 ? (0, token_1.TokenColumnProvider)(dict_typeTokens)(db_1.Column.Schema.str) : undefStr,
        chain: maxColumnCount > 5 ? (0, token_1.TokenColumnProvider)(chainTokens)(db_1.Column.Schema.str) : undefStr,
        sub_type: maxColumnCount > 6 ? (0, token_1.TokenColumnProvider)(sub_typeTokens)(db_1.Column.Schema.str) : undefStr,
        inter_bonds: maxColumnCount > 7 ? (0, token_1.TokenColumnProvider)(inter_bondsTokens)(db_1.Column.Schema.int) : undefInt,
        status_bits: maxColumnCount > 8 ? (0, token_1.TokenColumnProvider)(status_bitsTokens)(db_1.Column.Schema.str) : undefStr,
    };
    return ret;
}
