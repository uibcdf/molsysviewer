...
const dof_frag_1 = require("../../mol-gl/shader/dof.frag.js");
...
class DofPass {
    ...
    update(camera, input, depthOpaque, depthTransparent, props, sphere) {
        ...
        const distance = linear_algebra_1.Vec3.distance(camera.state.position, worldCenter);
        const inFocus = distance + props.inFocus;
        mol_util_1.ValueCell.updateIfChanged(this.renderable.values.uInFocus, inFocus * camera.scale);
        // transform center in view space
        const center = this.renderable.values.uCenter.ref.value;
        linear_algebra_1.Vec3.transformMat4(center, worldCenter, camera.view);
        ...
        mol_util_1.ValueCell.updateIfChanged(this.renderable.values.uPPM, props.PPM * camera.scale);
        if (needsUpdate) {
