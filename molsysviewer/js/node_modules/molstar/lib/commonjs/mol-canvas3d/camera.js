...
class Camera {
    ...
    update() {
        ...
        const distance = linear_algebra_1.Vec3.distance(snapshot.position, snapshot.target);
        const minTargetDistance = this.minTargetDistance / this.scale;
        if (distance < minTargetDistance) {
            linear_algebra_1.Vec3.sub(this.deltaDirection, snapshot.target, snapshot.position);
            linear_algebra_1.Vec3.setMagnitude(this.deltaDirection, this.deltaDirection, minTargetDistance);
            linear_algebra_1.Vec3.sub(snapshot.position, snapshot.target, this.deltaDirection);
        }
        const height = 2 * Math.tan(snapshot.fov / 2) * linear_algebra_1.Vec3.distance(snapshot.position, snapshot.target) * this.scale;
    ...
    getRay(out, x, y) {
        if (this.state.mode === 'orthographic') {
            linear_algebra_1.Vec3.set(out.origin, x, y, 0);
            this.unproject(out.origin, out.origin);
            linear_algebra_1.Vec3.normalize(out.direction, linear_algebra_1.Vec3.sub(out.direction, this.target, this.position));
            linear_algebra_1.Vec3.scaleAndAdd(out.origin, out.origin, out.direction, -this.near);
        }
        else {
            linear_algebra_1.Vec3.copy(out.origin, this.state.position);
            linear_algebra_1.Vec3.scale(out.origin, out.origin, this.scale);
    ...
    constructor(state, viewport = util_1.Viewport.create(0, 0, 128, 128)) {
        ...
        this.headRotation = linear_algebra_1.Mat4.zero();
        this.viewEye = linear_algebra_1.Mat4.zero();
...
const tmpPosition = (0, linear_algebra_1.Vec3)();
const tmpTarget = (0, linear_algebra_1.Vec3)();
function updateView(camera) {
    if (camera.scale === 1) {
        linear_algebra_1.Mat4.lookAt(camera.view, camera.state.position, camera.state.target, camera.state.up);
    }
    else {
        linear_algebra_1.Vec3.scale(tmpPosition, camera.state.position, camera.scale);
        linear_algebra_1.Vec3.scale(tmpTarget, camera.state.target, camera.scale);
        linear_algebra_1.Mat4.lookAt(camera.view, tmpPosition, tmpTarget, camera.state.up);
    }
}
...
function updateClip(camera) {
    ...
    linear_algebra_1.Vec3.scale(tmpTarget, camera.state.target, scale);
    linear_algebra_1.Vec3.scale(tmpPosition, camera.state.position, scale);
    const cameraDistance = linear_algebra_1.Vec3.distance(tmpPosition, tmpTarget);
