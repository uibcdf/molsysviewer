...
import { Download, ParseCif } from '../../mol-plugin-state/transforms/data.js';
import { CreateGroup } from '../../mol-plugin-state/transforms/misc.js';
import { VolumeFromSegmentationCif } from '../../mol-plugin-state/transforms/volume.js';
import { PluginCommands } from '../../mol-plugin/commands.js';
import { Color } from '../../mol-util/color/index.js';
import { BOX, MAX_VOXELS } from './entry-root.js';
...
export class VolsegLatticeSegmentationData {
    ...
    makeLoci(segments) {
        ...
        if (!wholeLoci || !Volume.Segment.isLoci(wholeLoci))
            return undefined;
        const elements = [{
                segments: SortedArray.ofUnsortedArray(segments),
                instances: Interval.ofLength(wholeLoci.volume.instances.length)
            }];
