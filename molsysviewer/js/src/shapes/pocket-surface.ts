...
function createGaussianSurfaceMeshes(structure: Structure, props: GaussianDensityProps, isoLevels: number[] | undefined, webgl?: any) {
...
export async function addPocketSurfaceFromPython(plugin: PluginContext, options: PocketSurfaceOptions) {
    ...
    const isoLevels = options.iso_levels && options.iso_levels.length ? options.iso_levels : [undefined];
    const isoAlphas = options.iso_alphas && options.iso_alphas.length === isoLevels.length ? options.iso_alphas : undefined;
    const isoColors = options.iso_colors && options.iso_colors.length === isoLevels.length ? options.iso_colors : undefined;
    ...
    for (let i = 0; i < meshes.length; i++) {
        ...
        const meshColor = isoColors
            ? new Map<number, Color>(Array.from({ length: options.atom_indices.length }, (_v, idx) => [idx, Color(isoColors[i])]))
            : colors;
        ...
        const data: PocketSurfaceData = {
            ...
            colors: meshColor,
            alpha,
            name: isoLevels[i] !== undefined ? `${baseName} (iso=${isoLevels[i]})` : baseName,
        };
