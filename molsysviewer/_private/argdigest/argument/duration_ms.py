...
_MESSAGE = ' A duration in milliseconds: either a bare number (250) or a quantity with explicit units ("250 ms").'
...
def digest_duration_ms(duration_ms, caller=None):
    ...
    if caller in _PLAIN_MILLISECOND_CALLERS:
        if isinstance(duration_ms, bool) or not isinstance(duration_ms, (int, float)):
            raise ArgumentError(
                "duration_ms", value=duration_ms, caller=caller, message="expected a duration in milliseconds"
            )
        if duration_ms <= 0:
            raise ArgumentError(
                "duration_ms",
    ...
    return digest_quantity(duration_ms, "duration_ms", {"[T]": 1}, caller=caller, message=_MESSAGE)
