...
La parte minima necesaria antes de 1.0 queda acotada al contrato basico MolSysMT -> ViewerJSON -> MolSysViewer:

- `formal_charge` via `MolecularMechanics` en MolSysMT y `atoms.formal_charge` en ViewerJSON;
...
- `formal_charge` se consume desde `ViewerJSON.atoms`, se serializa en el payload MolSysViewer y se materializa en TypeScript como `atom_site.pdbx_formal_charge`.
