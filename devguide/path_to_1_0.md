...
| 26 | **Architecture rework and hardening** | In progress. Phases 0a–6, 8 and 9 are audited; Phase 7 awaits two visible Qt observations. Phase 10 has closed notebook CI, state-file persistence and hover semantics; public-callable digestion, dependency-channel closure, final installed-artifact reruns, dogfooding and the final matrix remain. See [`pending_proposals/pre_1_0_architecture_rework_and_hardening_master_plan.md`](pre_1_0_architecture_rework_and_hardening_master_plan.md). |
| 27 | **Single-user remote session and rendering placement** | In progress. RRS0, RRS1 and RRS2 are complete. The server-rendered browser workflow has a managed MolSysViewer worker, array-native structures, authenticated VP8 video/input, UI-only workbench and trajectory, authoritative picking, upload/export, reconnect and clean termination. Client rendering uses the same authenticated gateway and exact full frontend entrypoint; its portable E2E proves local WebGL, real array-native structure, bounded reconnect without a second canvas, direct canvas camera/picking, authoritative Whole/selection/trajectory actions, local PNG export, canonical HTML download and authenticated replacement upload. RRS3 has a native Qt `--connect` shell that reuses the identical authenticated page, with native menus/shortcuts, upload activation and save-dialog-managed downloads. Visible aleph acceptance remains. RRS4 now has bounded automatic worker recovery, canonical scene retransmission, WebRTC renegotiation, a static-scene-safe video watchdog, visible lifecycle states and the guarded `molsysviewer-server` foreground CLI with versioned JSON startup and meaningful exit codes. The loopback gateway now guards origin/token authentication, failed-attempt and message sizes, malformed peers, input rate, session-specific cookies and cross-session artifacts. Clean installed-environment and visible aleph deployment closure remain. Deliver explicit `render_on="client" | "server"`, browser and Qt clients, one Python authority and the visible spika-to-aleph acceptance path. The CLI is explicitly not a daemon or multi-session manager. Remote collaboration, managed TURN, durable service management and cluster scheduling remain post-1.0. See [`remote_rendering_plan.md`](remote_rendering_plan.md). |
