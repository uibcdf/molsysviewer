...
def test_anywidget_binary_capability_never_builds_the_json_fallback(monkeypatch):
    ...
    begin, begin_buffers = next(item for item in sent if item[0].get("op") == "structure_data_begin")
    ...
    binary_message, buffers = next(item for item in sent if item[0].get("op") == "structure_data_chunk")
...
def test_anywidget_stream_sends_one_bounded_chunk_per_ack_and_releases_on_completion():
    ...
    for chunk_id in range(3):
        ...
        chunk_ids = [message["chunk_id"] for message, _ in sent if message["op"] == "structure_data_chunk"]
