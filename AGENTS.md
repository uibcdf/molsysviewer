...
10. JS/TS tests:
    ...
    - E2E in `molsysviewer/js/tests/e2e` (headless Playwright against real Mol*). `npm run test:e2e` runs all suites through one shared Chromium process. `npm run test:e2e:portable` runs the 36 browser scenarios that do not require a managed hardware-GPU render worker; `npm run test:e2e:server-gpu` runs that one distinct scenario on a qualified GPU host. The hosted `.github/workflows/CI_e2e.yaml` runs the portable lane and does not certify server rendering. You can force a local Chrome/Chromium with `PW_CHROMIUM_BIN=/path/to/chrome`. Browser launch or WebGL2 failure is an error by default; only an explicit `E2E_ALLOW_SKIP=1` opts out, and an opt-out is not validation. A stalled browser install is a CI provisioning failure, not evidence about the product. Visible-window Qt/GPU observations remain separate manual release evidence. Do not substitute xvfb/mesa for those observations.
