...
10. JS/TS tests:
    ...
    - E2E in `molsysviewer/js/tests/e2e` (headless Playwright against real Mol*). `npm run test:e2e` runs all suites through one shared Chromium process. You can force a local Chrome/Chromium with `PW_CHROMIUM_BIN=/path/to/chrome npm run test:e2e`. Browser launch or WebGL2 failure is an error by default; only an explicit `E2E_ALLOW_SKIP=1` opts out. Keep the headless browser suite in `.github/workflows/CI_e2e.yaml` as well as available for local diagnosis; a stalled browser install is a CI provisioning failure, not evidence about the product. Visible-window Qt/GPU observations remain separate manual release evidence. Do not substitute xvfb/mesa for those observations.
