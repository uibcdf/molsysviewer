...
**Unsupported preview; supported remote sessions are post-1.0.** The commands
below remain available for evaluation, but remote-client and managed server-GPU
E2E certification is not part of the 1.0 release gate. Their Python API,
command-line options, transport protocol and deployment configuration may
change in a later release. Do not build unattended or long-lived production
services against this interface.
...
as an authenticated actor. The preview API is not frozen; the explicit meaning
of client and server rendering remains an architectural design constraint.

The architectural invariants remain deliberate even while this feature is an
unsupported preview: Python is the single mutation authority, placement is explicit,
