# Developer guide

Use this section to understand how MolSysViewer is built.
You also use it as the source of truth for contributor workflows and docs style.

```{toctree}
:caption: Getting started
:maxdepth: 2

intro/index.md
dev_setup.md
repo_structure.md
configuration.md
demo_systems.md
contributing.md
ci.md
```

```{toctree}
:caption: Architecture
:maxdepth: 2

concepts.md
public_api.md
protocol_and_payloads.md
units_and_quantities.md
architecture.md
architecture_full.md
addons.md
regions_layers.md
debugging.md
```

```{toctree}
:caption: Workflow
:maxdepth: 2

js_workflow.md
testing.md
warnings.md
smonitor.md
releasing.md
```

```{toctree}
:caption: Documentation
:maxdepth: 2

docs.md
docs_workflow.md
documentation/web/index.md
documentation/api/index.md
```

```{toctree}
:caption: Style
:maxdepth: 2

style.md
type_annotations_guidelines.md
string_style.md
```

```{toctree}
:caption: Planning and history
:maxdepth: 2

roadmap.md
changes_notes.md
future/index.md
architecture_snapshot_2026_01.md
architecture_snapshot_2025_11.md
appendix/index.md
```
