...
- When reviewing docs with embedded views, serve the build: `python -m molsysviewer.tools.preview docs/_build/html`. Opening `index.html` from disk shows every embed blank, because a page with no origin may not load the shared runtime beside it.
